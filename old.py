...
class Detection: #detection class for neural network
    def __init__(self) :   
        ...
        self.ckpt.restore(os.path.join(paths['CHECKPOINT_PATH'], 'ckpt-8')).expect_partial()
    ...
    def start(self) :
        if self.started :
            print ("Detection model has already been initialized")
            return None
        self.started = True
        return self
    ...
    def detectionW(self, frame):
        currGesture , imageDetected = self.detection(frame) #run detection
        if currGesture == self.prevGesture :    #check for stable gesture (removes one off frame detections, reduces noisy detections)
            self.currGestureCount += 1
            if self.currGestureCount > 5 :
                self.currGestureCount = 0
                return currGesture, imageDetected
        else :          #unstable gesture, reset
            self.currGestureCount = 0
            self.prevGesture = currGesture
        return -1, frame #if either the gesture changed or it still isn't stable enough
    ...
    def detection(self,frame):
        ...
        if (max_val > 0.85) :
            ...
            gesture = detections['detection_classes'].__getitem__(max_idx)
        else :
            gesture = -1
...
#
    ...
    while (kill_capture==False):
       ...
       #    ans = -1
        #capture_stop = True
        ans, image_np_with_detections = detect.detectionW(frame)
        frame_resized = cv2.resize(image_np_with_detections, (540, 360))
